...
### Added
- **MCP Bridge Server** - Native Model Context Protocol support via `uac-mcp-server` npm package
- **WebSocket Streaming** - Real-time event streaming with 12 channels (console, hierarchy, selection, scene, etc.)

### Fixed
- **Locale Fix** - Fixed float parsing across all 16 handlers for non-English locales (decimal separator bug causing values like `2.5` to parse as `0`)
- Uses `CultureInfo.InvariantCulture` for all `float.TryParse` calls
